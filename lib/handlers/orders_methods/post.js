...
var callback, protocol, url, method, successCodes, timeoutSeconds, token, userEmail, userOrder, orderId, orderObject;
...
function Post (data, xd){
  callback = xd;
  // Validate inputs
  protocol = typeof(data.payload.protocol) == 'string' && ['https', 'http'].indexOf(data.payload.protocol) > -1 ? data.payload.protocol : false;
...
function postUsersRead (err, userData){
  if(!err && userData){
    userOrder = typeof(userData.order) == 'object' && userData.order instanceof Array ? userData.order : [];
